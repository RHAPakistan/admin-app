...
import DropoffList from '../../components/ButtonList/DropoffList';

const DropoffManagerScreen = ({ navigation }) => {
	...
	const onSubmit = (query) => {
		...
		console.log('Dropoff Searched', query);
	};

	const onPressHandler = (id) => {
		navigation.navigate('DropoffDetailsScreen', { id });
	};
		...
		<Pressable onPress={Keyboard.dismiss} style={GlobalStyles.container}>
			...
			<View style={GlobalStyles.screenTitle}>
				...
				<Pressable
					...
					onPress={() => navigation.push('CreateDropoffScreen')}>
					<Entypo name='plus' size={30} color={Colors.white} />
				</Pressable>
			</View>

			<Search onSubmit={onSubmit} placeholder='Search Dropoff' />

			<DropoffList data={data} onPress={onPressHandler} />
		</Pressable>
	);
};

export default DropoffManagerScreen;
