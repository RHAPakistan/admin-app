...
const VolunteerHistoryScreen = ({ navigation, route }) => {
		...
		useEffect(() => {
			...
			const onMount = navigation.addListener('focus', () => {
				...
				const fetchData = async () => {
					const resp = await adminApi.get_pickups({"volunteer":pickup._id});
		...
		const onChange = async (query) => {
			...
			if (query.index == 0) {
				...
				const response = await adminApi.get_pickups({"volunteer":pickup._id,"status": query.index - 1 });
		...
		return (
			<Pressable onPress={Keyboard.dismiss} style={GlobalStyles.container}>
				...
				<View style={GlobalStyles.screenTitle}>
					<Text style={GlobalStyles.screenTitleText}>Pickups for {pickup.fullName}</Text>
				</View>
